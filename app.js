...
window.openShippingModal = function () {
    ...
    if (!modal) {
        ...
        modal.innerHTML = `
            ...
            <div class="custom-footer-modal-content" style="background: #0f1622; border: 1px solid rgba(255,215,0,0.3); border-radius: 20px; width: 100%; max-width: 650px; max-height: 85vh; overflow-y: auto; padding: 2.5rem; position: relative; box-shadow: 0 25px 50px rgba(0,0,0,0.9);">
                <button onclick="window.closeShippingModal()" style="position: absolute; top: 1.25rem; right: 1.25rem; background: none; border: none; color: #94a3b8; font-size: 1.8rem; cursor: pointer; line-height: 1;">&times;</button>
                <div style="display: inline-block; background: rgba(255,215,0,0.1); border: 1px solid rgba(255,215,0,0.3); color: #ffd700; font-size: 0.75rem; font-weight: 800; padding: 0.3rem 0.8rem; border-radius: 20px; text-transform: uppercase; margin-bottom: 1rem;">Logistics</div>
                <h2 style="color: #fff; font-size: 1.7rem; margin-bottom: 1rem; font-weight: 800;">Shipping & Delivery Policy</h2>
                <div style="display: flex; flex-direction: column; gap: 1rem; margin-bottom: 1.5rem;">
                    <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; padding: 1.25rem;">
                        <h4 style="color: #ffd700; margin-bottom: 0.3rem; font-size: 1rem;">Free Express Delivery</h4>
                        <p style="color: #cbd5e1; font-size: 0.85rem; line-height: 1.5;">Complimentary express shipping across India on all orders exceeding ₹999.</p>
                    </div>
                    <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; padding: 1.25rem;">
                        <h4 style="color: #ffd700; margin-bottom: 0.3rem; font-size: 1rem;">Commercial Freight & White-Glove</h4>
                        <p style="color: #cbd5e1; font-size: 0.85rem; line-height: 1.5;">Heavy commercial rigs and multi-stations are transported via specialized hydraulic freight with scheduled delivery appointments.</p>
                    </div>
                </div>
                <div style="display: flex; gap: 1rem; justify-content: flex-end;">
                    <button onclick="window.closeShippingModal()" class="btn btn-gold glow-btn" style="padding: 0.65rem 1.5rem; font-size: 0.85rem;">Close</button>
                </div>
            </div>
        `;
        modal.addEventListener('click', (e) => {
            if (e.target === modal) window.closeShippingModal();
        });
        document.body.appendChild(modal);
    }
    modal.style.display = 'flex';
...
window.addBundleToCart = function (bundleKey) {
    ...
    if (window.auth && typeof auth.showToast === 'function') {
        auth.showToast(`${bundle.title} added to cart!`, 'success');
    } else if (typeof showToast === 'function') {
    ...
    // Open Cart Drawer so user immediately sees their added bundle
...
window.openBundleModal = function (bundleKey) {
            ...
            <div style="background: #0f1622; border: 1px solid rgba(255,215,0,0.3); border-radius: 20px; width: 100%; max-width: 650px; max-height: 90vh; overflow-y: auto; position: relative; box-shadow: 0 25px 50px rgba(0,0,0,0.9);">
                ...
                <div style="padding: 2rem;">
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 1.5rem;">
                        ...
                        <button id="bundle-modal-add-btn" class="btn btn-gold glow-btn" style="padding: 0.9rem 1.8rem; font-weight: 800; font-size: 0.95rem; cursor: pointer;">ADD BUNDLE TO CART</button>
                    </div>
                </div>
            </div>
        `;
    ...
    if (addBtn) {
        addBtn.onclick = function () {
            window.addBundleToCart(bundleKey);
        };
    }
